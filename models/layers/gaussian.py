...
from utils.boxEnlarge import enlargebox


class GaussianBuilder(object):
    def __init__(self, init_size, sigma, enlarge_region, enlarge_affinity, verbose=True):
    ...
    def add_gaussian_map_to_score_map(
        ...
        try:
            ...
            if self.verbose:
                print(f"Error : {e}, generating {map_type} map, strange box came out. (width: {width}, height: {height})")
            else:
                pass
        return score_map
    ...
    def generate_region(
        self, image_size, word_level_char_bbox, horizontal_text_bools
    ):
