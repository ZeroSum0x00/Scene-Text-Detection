...
class SaltAndPepper:
    ...
    def __call__(self, images):
        try:
            ...
            intensity_levels = 2 ** (images[0][0, 0].nbytes * 8)
            min_intensity, max_intensity = 0, intensity_levels - 1
            random_image = np.random.choice([min_intensity, 1, np.nan], p=[self.phi / 2, 1 - self.phi, self.phi / 2], size=images[0].shape)
            images[0] = images[0].astype(np.float32) * random_image
            images[0] = np.nan_to_num(images[0], nan=max_intensity).astype(dtype)
            return images
        except:
            return images
